  ...
  result.networks = userNetworks
    .map(network => {
      ...
      return {
        ...
        networkConfig,
        label,
        timeperiod: `Started at: ${moment(network.createdOn).format('DD-MMM-YYYY kk')} ${
          network.deletedAt ? ` to ${moment(network.deletedAt).format('DD-MMM-YYYY kk:ss')}` : 'and still running'
        }`,
      };
    })
    .filter(n => !!n);
