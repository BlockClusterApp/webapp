                        ...
                        {this.props.networks.map((item, index) => {
                          return (
                            <tr key={item._id} onClick={() => this.openNetwork(item.instanceId)}>
                              ...
                              <td className="v-align-middle">{helpers.firstLetterCapital(item.peerType)}</td>
                              <td className="v-align-middle">{this.getLocationName(item.locationCode)}</td>
                              <td className="v-align-middle">
                                {ReactHtmlParser(
                                  helpers.convertStatusToTag(
                                    helpers.calculateNodeStatus(item.status, item.lastPinged),
                                    helpers.firstLetterCapital(helpers.calculateNodeStatus(item.status, item.lastPinged))
                                  )
                                )}
                              </td>
