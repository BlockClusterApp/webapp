...
export default class EmailVerification extends Component {
  ...
  componentDidMount() {
    ...
    if (history.pushState) {
      const newurl =
        window.location.protocol + '//' + window.location.host + window.location.pathname + `?action=reset-password&id=${btoa(`${new Date().getTime()}-${queries.key}`)}`;
  ...
  checkVerificationCode() {
    Meteor.call('verifyResetPasswordLink', sessionStorage.getItem('key') || queryString(this.props.location.search).key, (err, res) => {
      if (!res) {
        this.setState({
          disabled: true,
          showMessage: true,
          isInvalidCode: true,
          message: (
            <p style={{ color: '#37b0e9' }}>
              Oops... Looks like the link has expired or is invalid. Kindly request for a{' '}
  ...
  submitToReset() {
    ...
    Meteor.call('changeUserPassword', sessionStorage.getItem('key'), this.password.value, (err, res) => {
      this.setState({
        disabled: true,
        showMessage: true,
        ...
        message: (
          <p>
            Congrats...!!! Your password has been changed successfully. You would be automatically redirected to login page in 5 seconds. Else <a href="/app/login"> Click here </a>
          </p>
        ),
      });
      setTimeout(() => {
    ...
    return (
      <div className="full-height">
        <div className="register-container full-height sm-p-t-30" style={{ marginTop: '50px' }}>
          ...
          />{' '}
          <br />
          <p style={this.state.isInvalidCode ? { color: '#aaa' } : {}}>We told you, we got your back. Just enter your new password and you are good to go.</p>
          ...
          >
            ...
            <span className="sm-block">
              <a href="https://www.blockcluster.io/terms" className="m-l-10 m-r-10" style={{ color: '#222', textDecoration: 'none' }}>
                Terms of use
              </a>
              <span className="muted">|</span>
              <a href="https://www.blockcluster.io/privacy" className="m-l-10" style={{ color: '#222', textDecoration: 'none' }}>
                Privacy Policy
              </a>
            </span>
          </p>
