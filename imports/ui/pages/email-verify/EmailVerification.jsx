...
export default class EmailVerification extends Component {
  ...
  componentDidMount() {
    ...
    if (history.pushState) {
      const newurl =
        window.location.protocol + '//' + window.location.host + window.location.pathname + `?action=verification&id=${btoa(`${new Date().getTime()}-${queries.key}`)}`;
    ...
    Meteor.call('emailVerification', queries.key, (err, res) => {
      if (res) {
        this.setState({
          ...
          resultMessage: (
            <p>
              Congrats...!!! Your email has been verified. You would be automatically redirected to login page in 5 seconds. Else <a href="/app/login"> Click here </a>
            </p>
          ),
        });
        setTimeout(() => {
  ...
  render() {
    const LoadingView = (
      <div className="d-flex justify-content-center flex-column full-height ">
        ...
        <div id="loader" />
        <br />
        <p style={{ textAlign: 'center', fontSize: '1.2em' }}>Hold on... We are validating the email id...</p>
      </div>
    );

    const InvalidView = (
      <div className="d-flex justify-content-center flex-column full-height " style={{ textAlign: 'center' }}>
        <img src="assets/img/logo/blockcluster.png" alt="logo" width="250" />

        <i className="fa fa-warning" style={{ color: '#d40000', fontSize: '7em', marginTop: '15px' }} />

        <div className="alert alert-danger" style={{ textAlign: 'center', marginTop: '15px' }}>
          Oooppsss... <br />
    ...
    const RedirectingView = (
      <div className="d-flex justify-content-center flex-column full-height " style={{ textAlign: 'center' }}>
        ...
        <i className="fa fa-check-circle" style={{ color: '#34db90', fontSize: '7em', marginTop: '15px' }} />

        <div className="alert alert-success" style={{ textAlign: 'center', marginTop: '15px' }}>
    ...
    return (
      <div className="full-height accept-invite">
        <div className="register-container full-height sm-p-t-30">{RenderView}</div>
      </div>
    );
